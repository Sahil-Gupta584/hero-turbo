import { defaultVideoDesc, defaultVideoTitle } from "@/app/constants";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const rawEditorsId = formData.get("editorsId");
    const editorsId =
      rawEditorsId === "undefined" || rawEditorsId === "null"
        ? null
        : (JSON.parse(formData.get("editorsId") as string) as string[]);
    const channelId = formData.get("channelId") as string;

    console.log("formData", formData);

    if (!videoFile) {
      return NextResponse.json({ ok: false, error: "Video file not found." });
    }
    ...
    const lastChunkRes = prisma.$transaction(async () => {
      ...
      if (lastChunkRes && lastChunkRes.ok) {
        ...
        if (editorsId) {
          for (const editorId of editorsId) {
            const videoEditor = await prisma.videoEditor.create({
              data: { videoId: res.id, editorId },
            });
          }
        }
      }
